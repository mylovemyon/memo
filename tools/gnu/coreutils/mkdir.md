...
GNU coreutils 9.5                                                                                               October 2024                                                                                                       MKDIR(1)
```
...
DESCRIPTION
       ...
       Whether other mode bits are honored for the created directory depends on the operating system.  For Linux, see NOTES below.
       ...
       or, synonymously mount -o grpid), the new directory will inherit the group ownership from its parent; otherwise it will be owned by the effective group ID of the process.
   ...
   mkdirat()
       ...
       by mkdir() for a relative pathname).
...
VERSIONS
   ...
   glibc notes
       On older kernels where mkdirat() is unavailable, the glibc wrapper function falls back to the use of mkdir().  When pathname is a relative pathname, glibc constructs a pathname based on the symbolic link  in  /proc/self/fd  that
