...
## applicationsinfo
```sh
*Evil-WinRM* PS C:\Users\Administrator\Documents> .\winPEASx64.exe quiet applicationsinfo
 [!] If you want to run the file analysis checks (search sensitive information in files), you need to specify the 'fileanalysis' or 'all' argument. Note that this search might take several minutes. For help, run winpeass.exe --help
ANSI color bit for Windows is not set. If you are executing this from a Windows terminal inside the host you should run 'REG ADD HKCU\Console /v VirtualTerminalLevel /t REG_DWORD /d 1' and then start a new CMD
Long paths are disabled, so the maximum length of a path supported is 260 chars (this may cause false negatives when looking for files). If you are admin, you can enable it with 'REG ADD HKLM\SYSTEM\CurrentControlSet\Control\FileSystem /v VirtualTerminalLevel /t REG_DWORD /d 1' and then start a new CMD
  WinPEAS-ng by @hacktricks_live
...
È Check if you can modify installed software https://book.hacktricks.wiki/en/windows-hardening/windows-local-privilege-escalation/index.html#applications
    C:\Program Files\Common Files(Administrators [Allow: WriteData/CreateFiles])
    C:\Program Files\desktop.ini(Administrators [Allow: AllAccess])
    C:\Program Files\internet explorer(Administrators [Allow: WriteData/CreateFiles])
    C:\Program Files\Uninstall Information(Administrators [Allow: AllAccess])
    C:\Program Files\VMware(Administrators [Allow: AllAccess])
    C:\Program Files\Windows Defender(Administrators [Allow: WriteData/CreateFiles])
    C:\Program Files\Windows Defender Advanced Threat Protection(Administrators [Allow: WriteData/CreateFiles])
    C:\Program Files\Windows Mail(Administrators [Allow: WriteData/CreateFiles])
    C:\Program Files\Windows Media Player(Administrators [Allow: WriteData/CreateFiles])
    C:\Program Files\Windows Multimedia Platform(Administrators [Allow: WriteData/CreateFiles])
    C:\Program Files\windows nt(Administrators [Allow: WriteData/CreateFiles])
    C:\Program Files\Windows Photo Viewer(Administrators [Allow: WriteData/CreateFiles])
    C:\Program Files\Windows Portable Devices(Administrators [Allow: WriteData/CreateFiles])
    C:\Program Files\Windows Security(Administrators [Allow: WriteData/CreateFiles])
    C:\Program Files\Windows Sidebar(Administrators [Allow: WriteData/CreateFiles])
    C:\Program Files\WindowsApps
    C:\Program Files\WindowsPowerShell(Administrators [Allow: WriteData/CreateFiles])
...
È Check if you can modify other users AutoRuns binaries (Note that is normal that you can modify HKCU registry and binaries indicated there) https://book.hacktricks.wiki/en/windows-hardening/windows-local-privilege-escalation/privilege-escalation-with-autorun-binaries.html                                                                                                                                                                                                       
   ...
   =================================================================================================
    ...
    Key: StubPath
    Folder: \
    FolderPerms: Administrators [Allow: AllAccess], Users [Allow: AppendData/CreateDirectories]
    File: /UserInstall
   =================================================================================================
...
È Check if you can modify other users scheduled binaries https://book.hacktricks.wiki/en/windows-hardening/windows-local-privilege-escalation/privilege-escalation-with-autorun-binaries.html
    ...
    Permissions folder(DLL Hijacking): Administrators [Allow: WriteData/CreateFiles]
    Trigger: When the task is created or modified

   =================================================================================================

    (EGOTISTICALBANK\Administrator) User_Feed_Synchronization-{07237857-F466-473F-A126-113C1649C712}: C:\Windows\system32\msfeedssync.exe sync
    Permissions folder(DLL Hijacking): Administrators [Allow: WriteData/CreateFiles]
    Trigger: At 3:51 PM every day-Trigger expires at 7/26/2031 3:51:06 PM.
   =================================================================================================
