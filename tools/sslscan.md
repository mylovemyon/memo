## SSL/TLS Protocols
```sh
└─$ sslscan --no-check-certificate --no-ciphersuites --no-compression --no-fallback --no-groups --no-heartbleed --no-renegotiation --verbose 10.129.229.183 
Version: 2.1.5
OpenSSL 3.5.0 8 Apr 2025

Connected to 10.129.229.183

Some servers will fail to response to SSLv3 ciphers over STARTTLS
If your scan hangs, try using the --tlsall option

Testing SSL server 10.129.229.183 on port 443 using SNI name 10.129.229.183

  SSL/TLS Protocols:
SSLv2     disabled
SSLv3     enabled
TLSv1.0   enabled
TLSv1.1   disabled
TLSv1.2   disabled
TLSv1.3   disabled
```


## SSL Certificate
```sh
└─$ sslscan --no-ciphersuites --no-compression --no-fallback --no-groups --no-heartbleed --no-renegotiation --verbose 10.129.229.183 
Version: 2.1.5
OpenSSL 3.5.0 8 Apr 2025

Connected to 10.129.229.183

Some servers will fail to response to SSLv3 ciphers over STARTTLS
If your scan hangs, try using the --tlsall option

Testing SSL server 10.129.229.183 on port 443 using SNI name 10.129.229.183

  SSL/TLS Protocols:
省略

  SSL Certificate:
Signature Algorithm: sha1WithRSAEncryption
RSA Key Strength:    1024

Subject:  localhost.localdomain
Issuer:   localhost.localdomain

Not valid before: Apr  7 08:22:08 2017 GMT
Not valid after:  Apr  7 08:22:08 2018 GMT
```


## Supported Server Cipher(s)
```
└─$ sslscan --no-check-certificate --no-compression --no-fallback --no-groups --no-heartbleed --no-renegotiation --verbose 10.129.229.183 
Version: 2.1.5
OpenSSL 3.5.0 8 Apr 2025

Connected to 10.129.229.183

Some servers will fail to response to SSLv3 ciphers over STARTTLS
If your scan hangs, try using the --tlsall option

Testing SSL server 10.129.229.183 on port 443 using SNI name 10.129.229.183

  SSL/TLS Protocols:
 省略

  Supported Server Cipher(s):
SSL_connect() returned: 1
Preferred TLSv1.0  256 bits  DHE-RSA-AES256-SHA            DHE 1024 bits
SSL_connect() returned: 1
Accepted  TLSv1.0  128 bits  DHE-RSA-AES128-SHA            DHE 1024 bits
SSL_connect() returned: 1
Accepted  TLSv1.0  256 bits  AES256-SHA                   
SSL_connect() returned: 1
Accepted  TLSv1.0  128 bits  AES128-SHA                   
SSL_connect() returned: -1
SSL_get_current_cipher() returned NULL; this indicates that the server did not choose a cipher from our list (ALL:COMPLEMENTOFALL:!DHE-RSA-AES256-SHA:!DHE-RSA-AES128-SHA:!AES256-SHA:!AES128-SHA)
Accepted  TLSv1.0  128 bits  TLS_RSA_WITH_RC4_128_MD5     
Accepted  TLSv1.0  128 bits  TLS_RSA_WITH_RC4_128_SHA     
Accepted  TLSv1.0  56 bits   TLS_RSA_WITH_DES_CBC_SHA     
Accepted  TLSv1.0  112 bits  TLS_RSA_WITH_3DES_EDE_CBC_SHA
Accepted  TLSv1.0  56 bits   TLS_DHE_RSA_WITH_DES_CBC_SHA 
Accepted  TLSv1.0  112 bits  TLS_DHE_RSA_WITH_3DES_EDE_CBC_SHA
```


## TLS Compression
```sh
└─$ sslscan --no-check-certificate --no-ciphersuites --no-fallback --no-groups --no-heartbleed --no-renegotiation --verbose 10.129.229.183 
Version: 2.1.5
OpenSSL 3.5.0 8 Apr 2025

Connected to 10.129.229.183

Some servers will fail to response to SSLv3 ciphers over STARTTLS
If your scan hangs, try using the --tlsall option

Testing SSL server 10.129.229.183 on port 443 using SNI name 10.129.229.183

  SSL/TLS Protocols:
省略

  TLS Compression:
OpenSSL OpenSSL 3.5.0 8 Apr 2025 looks like version 0.9.8m or later; I will try SSL_OP to enable renegotiation
Compression enabled (CRIME)
```


## TLS Fallback SCSV
```sh
└─$ sslscan --no-check-certificate --no-ciphersuites --no-compression --no-groups --no-heartbleed --no-renegotiation --verbose 10.129.229.183 
Version: 2.1.5
OpenSSL 3.5.0 8 Apr 2025

Connected to 10.129.229.183

Some servers will fail to response to SSLv3 ciphers over STARTTLS
If your scan hangs, try using the --tlsall option

Testing SSL server 10.129.229.183 on port 443 using SNI name 10.129.229.183

  SSL/TLS Protocols:
省略

  TLS Fallback SCSV:
OpenSSL OpenSSL 3.5.0 8 Apr 2025 looks like version 0.9.8m or later; I will try SSL_OP to enable renegotiation
OpenSSL OpenSSL 3.5.0 8 Apr 2025 looks like version 0.9.8m or later; I will try SSL_OP to enable renegotiation
Server does not support TLS Fallback SCSV
```


## Heartbleed
```sh
└─$ sslscan --no-check-certificate --no-ciphersuites --no-compression --no-fallback --no-groups --no-renegotiation --verbose 10.129.229.183 
Version: 2.1.5
OpenSSL 3.5.0 8 Apr 2025

Connected to 10.129.229.183

Some servers will fail to response to SSLv3 ciphers over STARTTLS
If your scan hangs, try using the --tlsall option

Testing SSL server 10.129.229.183 on port 443 using SNI name 10.129.229.183

  SSL/TLS Protocols:
省略

  Heartbleed:
TLSv1.0 not vulnerable to heartbleed
```


## TLS renegotiation
```sh
└─$ sslscan --no-check-certificate --no-ciphersuites --no-compression --no-fallback --no-heartbleed --verbose 10.129.229.183   
Version: 2.1.5
OpenSSL 3.5.0 8 Apr 2025

Connected to 10.129.229.183

Some servers will fail to response to SSLv3 ciphers over STARTTLS
If your scan hangs, try using the --tlsall option

Testing SSL server 10.129.229.183 on port 443 using SNI name 10.129.229.183

  SSL/TLS Protocols:
省略

  TLS renegotiation:
OpenSSL OpenSSL 3.5.0 8 Apr 2025 looks like version 0.9.8m or later; I will try SSL_OP to enable renegotiation
use_unsafe_renegotiation_op
Attempting secure_renegotiation_support()
Secure session renegotiation supported
```


## SSL Certificate
```sh
└─$ sslscan --show-certificate --no-check-certificate --no-ciphersuites --no-compression --no-fallback --no-groups --no-heartbleed --no-renegotiation --verbose 10.129.229.183
Version: 2.1.5
OpenSSL 3.5.0 8 Apr 2025

Connected to 10.129.229.183

Some servers will fail to response to SSLv3 ciphers over STARTTLS
If your scan hangs, try using the --tlsall option

Testing SSL server 10.129.229.183 on port 443 using SNI name 10.129.229.183

  SSL/TLS Protocols:
省略

sslMethod = TLS_method()

  SSL Certificate: 
    Certificate blob:
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
    Version: 2
    Serial Number: 32085 (0x7d55)
    Signature Algorithm: sha1WithRSAEncryption
    Issuer: /C=--/ST=SomeState/L=SomeCity/O=SomeOrganization/OU=SomeOrganizationalUnit/CN=localhost.localdomain/emailAddress=root@localhost.localdomain
    Not valid before: Apr  7 08:22:08 2017 GMT
    Not valid after: Apr  7 08:22:08 2018 GMT
    Subject: /C=--/ST=SomeState/L=SomeCity/O=SomeOrganization/OU=SomeOrganizationalUnit/CN=localhost.localdomain/emailAddress=root@localhost.localdomain
    Public Key Algorithm: NULL
    RSA Public Key: (1024 bit)
      RSA Public-Key: (1024 bit)
      Modulus:
          00:b7:7b:81:e1:2d:83:cd:83:02:78:78:a9:56:fd:
          4a:66:7a:63:df:2b:f6:b7:99:c6:9f:4a:ac:7f:00:
          93:f7:e0:e0:b4:4f:08:ff:71:93:5a:a8:53:cb:39:
          67:cd:8d:ca:b8:9a:68:95:fb:ef:5a:a0:90:a4:8f:
          86:a9:0e:4c:88:08:8d:5e:9a:ca:cf:69:17:6b:94:
          0f:dc:09:77:27:de:7b:a9:7c:e8:1c:7e:90:55:e2:
          fa:8d:7e:aa:2b:12:aa:49:08:ee:d3:e1:b7:29:67:
          a6:07:bb:1b:51:b4:fe:e0:44:98:84:d7:2b:40:14:
          fd:a3:0e:fb:6f:32:a1:16:93
      Exponent: 65537 (0x10001)
    X509v3 Extensions:
      X509v3 Subject Key Identifier: 
        BF:CE:2C:9E:E1:25:57:9D:94:BE:46:93:47:D8:BE:8E:9B:66:4A:96
      X509v3 Authority Key Identifier: 
        keyid:BF:CE:2C:9E:E1:25:57:9D:94:BE:46:93:47:D8:BE:8E:9B:66:4A:96
        DirName:/C=--/ST=SomeState/L=SomeCity/O=SomeOrganization/OU=SomeOrganizationalUnit/CN=localhost.localdomain/emailAddress=root@localhost.localdomain
        serial:7D:55
      X509v3 Basic Constraints: 
        CA:TRUE
  Verify m:
    self-signed certificate
```


## Acceptable client certificate CA names
```sh
└─$ sslscan --show-client-cas --no-check-certificate --no-ciphersuites --no-compression --no-fallback --no-groups --no-heartbleed --no-renegotiation --verbose 10.129.229.183
Version: 2.1.5
OpenSSL 3.5.0 8 Apr 2025

Connected to 10.129.229.183

Some servers will fail to response to SSLv3 ciphers over STARTTLS
If your scan hangs, try using the --tlsall option

Testing SSL server 10.129.229.183 on port 443 using SNI name 10.129.229.183

  SSL/TLS Protocols:
 省略

sslMethod = TLS_method()

  Acceptable client certificate CA names:
None defined (any)
```


## Server Signature Algorithm(s)
```sh
└─$ sslscan --show-sigs --no-check-certificate --no-ciphersuites --no-compression --no-fallback --no-groups --no-heartbleed --no-renegotiation --verbose 10.129.229.183
Version: 2.1.5
OpenSSL 3.5.0 8 Apr 2025

Connected to 10.129.229.183

Some servers will fail to response to SSLv3 ciphers over STARTTLS
If your scan hangs, try using the --tlsall option

Testing SSL server 10.129.229.183 on port 443 using SNI name 10.129.229.183

  SSL/TLS Protocols:
省略

  Server Signature Algorithm(s):
TLSv1.0  Server accepts all signature algorithms.
```
